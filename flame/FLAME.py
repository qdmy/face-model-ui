...
class FLAME(nn.Module):
    ...
    def __init__(self, config):
        ...
        with open(config.face_model_path, 'rb') as f:
        ...
        self.faces = self.flame_model.f # 面片
        ...
        default_shape = torch.zeros([self.batch_size, 300-config.shape_params], # 形状基系数初始化
        ...
        default_exp = torch.zeros([self.batch_size, 100 - config.expression_params], # 表情基系数初始化
                                    dtype=self.dtype, requires_grad=False)
        self.register_parameter('expression_betas', nn.Parameter(default_exp,
                                                            requires_grad=False))

        default_root_pose = torch.zeros([self.batch_size, 3], # root pose基系数初始化
                                    dtype=self.dtype, requires_grad=False)
        self.register_parameter('root_pose_betas', nn.Parameter(default_root_pose, requires_grad=False))

        # Eyeball and neck rotation
        default_eyball_pose = torch.zeros([self.batch_size, 6], # 眼球和脖子的旋转基系数
                                    dtype=self.dtype, requires_grad=False)
        ...
        self.register_buffer('v_template', # mesh模板
                             to_tensor(to_np(self.flame_model.v_template),
                                       dtype=self.dtype))

        # The shape components
        shapedirs = self.flame_model.shapedirs # 形状+表情基
        # The shape components
        self.register_buffer(
            'shapedirs',
            to_tensor(to_np(shapedirs), dtype=self.dtype))

        j_regressor = to_tensor(to_np( # 根据顶点得到joint的回归器
            self.flame_model.J_regressor), dtype=self.dtype)
        self.register_buffer('J_regressor', j_regressor)

        # Pose blend shape basis
        num_pose_basis = self.flame_model.posedirs.shape[-1]
        posedirs = np.reshape(self.flame_model.posedirs, [-1, num_pose_basis]).T # 姿态基
        self.register_buffer('posedirs',
                             to_tensor(to_np(posedirs), dtype=self.dtype))

        # indices of parents for each joints
        parents = to_tensor(to_np(self.flame_model.kintree_table[0])).long() # joints间的关系
        parents[0] = -1
        self.register_buffer('parents', parents)

        self.register_buffer('lbs_weights',
                             to_tensor(to_np(self.flame_model.weights), dtype=self.dtype)) # 每个joint对所有顶点的影响

        # Static and Dynamic Landmark embeddings for FLAME

        with open(config.static_landmark_embedding_path, 'rb') as f:
            static_embeddings = Struct(**pickle.load(f, encoding='latin1')) # 脸部的51个关键点

        lmk_faces_idx = (static_embeddings.lmk_face_idx).astype(np.int64) # 关键点所在的面片index
        self.register_buffer('lmk_faces_idx',
                             torch.tensor(lmk_faces_idx, dtype=torch.long))
        lmk_bary_coords = static_embeddings.lmk_b_coords # 关键点所在面片的重心坐标
        self.register_buffer('lmk_bary_coords',
                             torch.tensor(lmk_bary_coords, dtype=self.dtype))

        if self.use_face_contour: # 轮廓点会随着转动变化，所以是一个序列，描述一个范围内的动态轮廓点
    ...
    def _find_dynamic_lmk_idx_and_bcoords(self, vertices, pose, dynamic_lmk_faces_idx,
        ...
        aa_pose = torch.index_select(pose.view(batch_size, -1, 3), 1,
                                     neck_kin_chain) # 选出对应neck的joint链
        rot_mats = batch_rodrigues(
            aa_pose.view(-1, 3)).view(batch_size, -1, 3, 3) # 把neck上的joint对应的轴角表示转换为旋转矩阵

        rel_rot_mat = torch.eye(3, device=vertices.device,
                                dtype=dtype).unsqueeze_(dim=0).expand(batch_size, -1, -1)
        for idx in range(len(neck_kin_chain)):
            rel_rot_mat = torch.bmm(rot_mats[:, idx], rel_rot_mat) # 由一个单位矩阵算出每个joint的相对旋转矩阵

        y_rot_angle = torch.round(
            torch.clamp(-rot_mat_to_euler(rel_rot_mat) * 180.0 / np.pi,
                        max=39)).to(dtype=torch.long) # 根据相对旋转矩阵算出对应的y轴的旋转欧拉角
        neg_mask = y_rot_angle.lt(0).to(dtype=torch.long) 
        mask = y_rot_angle.lt(-39).to(dtype=torch.long)
        neg_vals = mask * 78 + (1 - mask) * (39 - y_rot_angle) # 78对应的就是动态landmark里考虑了78种角度
        y_rot_angle = (neg_mask * neg_vals +
                       (1 - neg_mask) * y_rot_angle) # 得出当前姿态对应的是哪个欧拉角

        dyn_lmk_faces_idx = torch.index_select(dynamic_lmk_faces_idx, # 选出对应维度，就是这个pose对应的轮廓
                                               0, y_rot_angle)
    ...
    def forward(self, shape_params=None, expression_params=None, pose_params=None, neck_pose=None, eye_pose=None, transl=None):
        ...
        if shape_params == None:
            ...
            betas = torch.cat([shape_params,self.shape_betas, expression_params, self.expression_betas], dim=1) # 形状基+表情基系数
        neck_pose = (neck_pose if neck_pose is not None else self.neck_pose) # 脖子旋转基系数
        eye_pose = (eye_pose if eye_pose is not None else self.eye_pose) # 眼球旋转基系数
        transl = (transl if transl is not None else self.transl) # 是平移整个mesh的作用，因为RingNet里的mesh不是在同一位置
        if pose_params.shape[1]==3:
            full_pose = torch.cat([self.root_pose_betas, neck_pose, pose_params, eye_pose], dim=1) # 完整的姿态基系数
        else:
            full_pose = torch.cat([pose_params[:,:3], neck_pose, pose_params[:,3:], eye_pose], dim=1) # 完整的姿态基系数
        template_vertices = self.v_template.unsqueeze(0).repeat(self.batch_size, 1, 1) # 每帧对应一个mesh模板

        vertices, _ = lbs(betas, full_pose, template_vertices,
                               self.shapedirs, self.posedirs,
                               self.J_regressor, self.parents,
                               self.lbs_weights,) # dtype=self.dtype) # 线性混合蒙皮，即把mesh模板按照形状基系数+表情基系数+姿态基系数进行变化
        ...
        if self.use_face_contour:
            ...
            dyn_lmk_faces_idx, dyn_lmk_bary_coords = self._find_dynamic_lmk_idx_and_bcoords( # 根据当前的mesh模型和pose，得到对应的动态landmark
        ...
        landmarks = vertices2landmarks(vertices, self.faces_tensor, # 算出新的关键点坐标
                                             lmk_faces_idx,
                                             lmk_bary_coords)

        if self.use_3D_translation: # 如果有平移，再整体进行平移
